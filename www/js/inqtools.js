...
$(function() {
    ...
    $('div.results').qtip({
        prerender: true,
        content: {text:'Click to show Details'},
        position: {
            my:'bottom right',
            at:'top center',
            adjust: {y:-2}
    ...
    }).delegate('.search_result_row .toggle_icon','hover',function(e) {
        if (e.type == 'mouseenter') {
            $(this).closest('div.results')
                .qtip('option','content.text','Click to show Details')
...
function switchTool(optObj) {
    var cb = optObj.data('logocb');
    $('#header')
        .data('logo',optObj.data('logo') || 'side')
        .data('callback',(cb && cb in window) ? window[cb] : null)
        .delay(100)
        .queue(animateLogo);
...
function animateLogo() {
    ...
    if (o == 'center') {
        ...
        animateLogoToSide($(this).data('callback'));
    }
    $(this).dequeue();
}
